...
local g3d = g3d_tmp -- save a reference to g3d in case the user makes it non-global
...
--[[setmetatable(fpsController, {
    __newindex = function(t, k, v)
        if k == "pitch" then
            __pitch = v
            print("Setting pitch to "..v)
...
camera.fpsController = fpsController
...
local distance = -8000
function camera.setTargetAndRotation(x,y,z,angle_radians)
    fpsController.pitch = -math.pi/4
    local offsetx, offsety, offsetz =
        math.cos(angle_radians) * distance,
        math.sin(angle_radians) * distance,
        distance*math.sin(fpsController.pitch)
    camera.target[1], camera.target[2], camera.target[3] = x, y, z
    fpsController.direction = angle_radians
    -- g3d.camera.lookInDirection(offsetx+x, offsety+y, offsetz+z, angle_radians, fpsController.pitch)
    g3d.camera.lookAt(x+offsetx,y+offsety,z+offsetz,x, y, z)
end
