...
function LightTransition:init(x, y, map)
	...
	self.cutsceneSettings = {
		fake_layer = Game.world.player.layer,
		fake_speed_y = 3,
		fake_gravity = -0.2,
...
function LightTransition.cutscene(cutscene, self, player)
	...
	cutscene:walkTo(kris, event.x + distance, event.y + event.height + 40, wait, 'down', true)
	cutscene:walkTo(susie, event.x - distance, event.y + event.height+ 40, wait, 'down', true)
	cutscene:walkTo(jamm, event.x, event.y + event.height+ 40, wait, 'down', true)
	...
	fakeKris.color = {0.4, 0, 0}

	-- sus
	susie.visible = false
	local fakeSusie = Sprite("party/susie/dark_transition/dark", susie.x - 29, susie.y - 86)
	...
	fakeSusie.color = {0.4, 0, 0}

	-- jam
	jamm.visible = false
	local fakeJamm = Sprite("party/jamm/dark_transition/dark", jamm.x - 29, jamm.y - 86)
	fakeJamm.physics.speed_y = settings.fake_speed_y
	fakeJamm.physics.gravity = settings.fake_gravity
	fakeJamm.physics.speed_y = settings.fake_speed_y
	fakeJamm.physics.gravity = settings.fake_gravity

	--fakeKris:setOrigin(0.5, 0.5)
	fakeJamm:setScale(2, 2)
	fakeJamm:play(1 / 15, true)

	fakeJamm.layer = settings.fake_layer

	Game.world:addChild(fakeJamm)
	fakeJamm.color = {0.4, 0, 0}
