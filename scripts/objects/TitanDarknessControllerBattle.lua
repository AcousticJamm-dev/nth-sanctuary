...
function TitanDarknessControllerBattle:init(data)
    ...
    for i = 1, 8 do
        table.insert(self.fumes, {MathUtils.random(0, SCREEN_WIDTH*2), SCREEN_HEIGHT-100, Utils.random(20, 40), self.timer + MathUtils.random(-30, 30)})
    end
end
function TitanDarknessControllerBattle:update()
    ...
    if self.spawn_timer < 0 then
        ...
        table.insert(self.fumes, {MathUtils.random(0, SCREEN_WIDTH*2), SCREEN_HEIGHT-100, MathUtils.random(20, 50), self.timer})
    end
