...
function ClimbWaterBucket:init(data)
    ...
    self.watertype = properties["watertype"] or 1
    self.generator = properties["generate"] or false
	self:setOrigin(0, 0.5)
	if self.watertype == 3 then
		self:setSprite("world/events/climbwater/climb_lavabucket")
	else
	...
	self.waterbad = properties["waterbad"] or false
	self.waterdmg = properties["waterdmg"] or 0
				...
				if waterspawntype == 1 then
					if MathUtils.round(self.timer) == self.waittime - 6 then
						if self.world.map.cyltower then
							...
							if self.watertype == 3 then
								splash:setSprite("world/events/climbwater/climb_lavabucket_splash")
							end
					...
					if MathUtils.round(self.timer) == self.waittime then
						local water = ClimbWater(self.x, self.y, self.watertype, self.watermovetimer,
						self.watermoverate, self.watertilelimit, self.waterfallingtimer,
						self.waterdir, self.spawnrate, self.activetime, self.waterbad, self.waterdmg)
		...
		if tile.vis == 1 then
			...
			if self.generator then
				...
				if self.drawwater > 0 then
					...
					if self.watertype == 3 then
						sprite = Assets.getFrames("world/events/climbwater/climb_lavabucket_splash")
					end
