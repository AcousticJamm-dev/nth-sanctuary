...
function WingladeActorSprite:update()
    ...
    if Game.world.player and Game.world.player:isMovementEnabled() then
        if anim == "spin" then
			...
			if self.afterimage_timer >= 2 then
				self:addChild(AfterImage(self, 0.25, 0.025))
				self.afterimage_timer = 0
			end
			self.eye_pupil.visible = true
			self.eye_white.x = MathUtils.lerp(self.eye_white.x, self.eye_target_x / 5, 1 - (1 - 0.2) ^ DTMULT)
			self.eye_white.y = MathUtils.lerp(self.eye_white.y, self.eye_target_y / 5, 1 - (1 - 0.2) ^ DTMULT)
			self.eye_pupil.x = MathUtils.lerp(self.eye_pupil.x, self.eye_target_x, 1 - (1 - 0.2) ^ DTMULT)
			self.eye_pupil.y = MathUtils.lerp(self.eye_pupil.y, self.eye_target_y, 1 - (1 - 0.2) ^ DTMULT)
            return
        end
    end
