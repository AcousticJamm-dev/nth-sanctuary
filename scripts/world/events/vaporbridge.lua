...
function VaporBridge:setupLogs()
	...
	for i = 0, self.log_amount do
		table.insert(self.logs, {index = self.log_amount - i, x = 0 + 20 * i, y = 0, bend_off = 0, bend_val = (i < math.floor(self.log_amount * 0.5) and (i + 1) * 2 or (self.log_amount - i) * 2)})
	end
end
...
function VaporBridge:update()
	...
	for _, chara in ipairs(Game.stage:getObjects(Character)) do
		...
		if chara:collidesWith(self.collider) then
			...
			apply_bend = true
			local chara_log_index = MathUtils.clamp(((chara.x - self.x) / 20) + 1, 0, self.log_amount + 1)
			local log_index = MathUtils.clamp(((chara.x - self.x) / 20) + 1, 1, self.log_amount)
